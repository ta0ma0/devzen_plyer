...
def get_all_podcasts(url_podcast, file_name_podcast ): #Получаем подкаст, для использования в цикле.
...
def list_day_podcasts():
    """Вывод списка подкастов на главной создает список headers_menu"""
...
def get_mp3_from_page(page_var):
    ...
    if get_audio is None:
        get_audio = soup.find_all('audio') #Для старой версии плеера
    podcast_link = ""
